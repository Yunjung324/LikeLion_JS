...
let s = new Set('abcdeeeeeeeee');  //중복을 제거한다
//{'a', 'b', 'c', 'd'}
s.size

회사게시판 = ['이호준', '이호준', '이호준', '이호준', '이호준', '이호준', '김연하', '김연하', '최흥석', '이나영']
...
let s = new Set('aabbcccdd'.split(''))
let s = new Set(['a', 'a', 'b', 'b', 'b', 'c', 'c', 'c', 'd', 'd'])

//교집합
let a = new Set('abc')
let b = new Set('cde')
let cro = [...a].filter(e => b.has(e)) 
//배열 a를 순환을 돌면서 b의 set에서 가지고 있는가를 보는 것

//집합은 iterable 객체(순회를 하는 객체)
//합집합


// set, map, object, array, string가 이터러블 객체인지? 순서는 보장하는지?
// 이터러블 객체란? 순회가 가능한 객체다
// set, map, array, string 모두 순회가 가능
