// 재귀함수의 중요도(*)
// 다이나믹 프로그램ㅇ, 메모라이징 기법 등을 사용하는 순간이 오게 되면 재귀함수의 중요도가 (**)

// 재귀함수 => 내가 나를 호출하는 것
// 반복문으로 반복할 수 있는 것은 재귀함수로 만들 수 있다.
// 재귀함수로 만들 수 있는 것은 반복문으로도 만들 수 있다.

//재귀함수를 굳이 쓰려고 하지말고 실무에서 쓰지말기, 실무에서는 반복문 사용하길 권함. (메모리 효율 등의 문제로)
//알고리즘 문제에서는 간혹 재귀를 호출해야할 일이 있음.
// (병합정렬, 퀵정렬)
...
factorial(5)    4 * factorial(4)    4 * 6 == 24
factorial(5)    3 * factorial(4)    3 * 2 == 6
factorial(2)    2 * factorial(1)    2 * 1 == 2
factorial(1)    1

//누적합(이렇게 일일이 순회하지 않고 구할 수 잇는 값이기 때문에 2개 다 비효율적인 코드. 특히 소수 구하는 문제 같은 곳에서 2중 for문을 사용하는 것도 매우 비효울적)
...
result  //이 코드는 그대로 hello world가 나옴

result = ''
for (const i of 'hello world') {
    result = i + result
}
result  //이 코드는 뒤집어져서 dlrow olleh가 나옴
...
reverse('hello world')  //이 코드도 그대로 hello world 출력
