...
// flag로 global을 주어서 전체 변경(m-대중라인, i-대소문자구분x)
...
// 오답 : 정규표현식 패턴은 문자열로 만드는 것이라 별도로 생성을 해주어야한다.
function solution(my_string, letter){
...
//숫자인 것을 뽑아내거나(match), 문자열을 지우거나(replace)
...
function solution(my_str, n) {
    let reg = new RegExp(`\\w{1,${n}}`, 'g')  // '\'를 표현하기 위해 \\사용
    return my_str.match(reg)
}
...
// 정답
'a10b9r10ce33uab8wc918v2cv11v9'.match(/([rev])(10|[0-9])/g).reduce
